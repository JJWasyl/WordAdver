...
if __name__=="__main__":
    ...
    while (epoch < n_epochs):
        ...
        if epoch == 1:
            ...
            train_set = np.row_stack([original_train_set,Train_set_add[1:,]])
            ...
            for m in xrange (batch_size):
                ...
                for i_vocab in xrange(n_option):
                    ...
                    if tag1 != tag2:
                        pair.update({w1+w2:-1})
                        continue                    
                    train_set_add=np.copy(train_set[index*batch_size:(index+1)*batch_size][m:m+1])
                    #print vec2sent(train_set_add)# output intermeidate results
                    ...
                    if np.array(sent_n1.nonzero()).size > 10:
                        ...
                        sec_w_2be_rep= int(sec_w_2be_rep[0][0])
                        if sec_w_2be_rep == 0:
                            break                                 
                        sec_v1=U[sec_w_2be_rep,].flatten()                
                        sec_w1=word_idx_map_invert[sec_w_2be_rep]
                        ...
                        for i_sec_vocab in xrange(n_option): 
                            ...
                            if sec_sim > sim_threshold:
                                continue
                            train_set_add[index*batch_size:(index+1)*batch_size][m:m+1,sec_din1_index[m]]=sec_din2_index[m]
                            if sec_w1 != 0:
                                break      
                    Train_set_add = np.row_stack([Train_set_add,train_set_add])
                    if w1 != 0: 
                        #print vec2sent(train_set_add)
                        break                               
